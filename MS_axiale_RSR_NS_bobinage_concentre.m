...
%                     (type NS) � deux rotors et 1 stator
%                     avec du bobinage concentr�
%
%  Auteur       :     Th�o NDEREYIMANA, Lucas Roberto DAGORT, Matheus
%                     Elyasha LOPES, Valentin GOLDIT�, Yoann ROUSSEL
%
...
volume_encoche = 0.6;                                                      % 60% du volume entre les p�les est destin� aux bobines
volume_dents   = 0.4;                                                      % 40% du volume entre les p�les est destin� aux dents
...
J = 8;           % [A/m^2]                                                 % Densit� de courant
Veff = 10;       % [Veff]                                                  % Tension d'alimentation (entr�e de la machine pique-pique)
Ieff = 10;       % [Aeff]
Imax_pique = 20; % [A]
psi = 0;         % [�]                                                     % D�phasage courant et tension d'entr�
N=10000;         % [rpm]                                                   % Vitesse de rotation d�sis�
f=P*N/60;        % [Hz]                                                    % Frequence champ tournante
Tperiod=1/f;     % [s]                                                     % Periode champ tournante
w=2*pi*f;        % [rad/s]                                                 % Vitesse champ tournante
pas = 60;
%--------------------------------------------------------------------------
% Param�tres Mat�rieux

mu0=4*pi*1e-7;                                                             % parametres des materiaux
Br0 = 1.16;                                                                % Induction remanente (20�C)
Hcb0 = 880*1e3;                                                            % Champ coercitive (20�C)
mua = Br0/(mu0*Hcb0);                                                      % Permeabilit� relative de l'AP
rho_cu = 1.72*1e-8;                                                        % R�sisitivit� du cuivre (ohm.m)
mvfer = 7800;                                                              % masse volumique du fer (kg/m3)
q = 1;                                                                     % Pertes fers sp�cifiques (W/kg)
...
Req = (Rext+Rint)/2;                                                       % Rayon �quivalent
lm = 2*pi*Req;                                                             % Longuer �quivalente
prof_eq = (Rext^2-Rint^2)/(2*Req);                                         % Profondeur �quivalente

deltay = 0;                                                                % Profondeur des aimants par rapport � la superfice du rotor
deltax = (lm/(2*P) - 2*pi*Req*(angle_aimant/360));                         % Distance entre deux aimants cons�cutifs
%deltax = 2;  %[mm]                                                        % Distance entre deux aimants cons�cutifs
delta_aimant = (2*pi*Req*(angle_aimant/360)+deltax);                       % Distance entre deux points �quivalents de deux aimants cons�cutifs
delta_dents =  (volume_dents*lm/(3*P));                                    % longueur d'un dent
delta_encoche = (volume_encoche*lm/(3*P));                                 % longueur d'un encoche

%% Rotor

NodeR(1,:) = [0 h_e+h_S/2];                                                % point en bas � gauche
NodeR(2,:) = [0 h_e+h_S/2+h_R];                                            % point en haut � gauche
NodeR(3,:) = [lm h_e+h_S/2];                                               % point en bas � droite
NodeR(4,:) = [lm h_e+h_S/2+h_R];                                           % point en haut � droite
...
NodeAIM(1,:) = [deltax/2 h_e+h_S/2+deltay];                                              % point en bas � gauche
NodeAIM(2,:) = [deltax/2 h_e+h_S/2+deltay+l_aimant];                                     % point en haut � gauche
NodeAIM(3,:) = [deltax/2+2*pi*Req*(angle_aimant/360) h_e+h_S/2+deltay];                  % point en bas � droite
NodeAIM(4,:) = [deltax/2+2*pi*Req*(angle_aimant/360) h_e+h_S/2+deltay+l_aimant];         % point en haut � droite
...
NodeSB(1,:) = [-0 -h_b/2];                                                 % point en bas � gauche
NodeSB(2,:) = [-0  h_b/2];                                                 % point en haut � gauche
NodeSB(3,:) = [delta_encoche*0.5  -h_b/2];                                 % point en bas � droite
NodeSB(4,:) = [delta_encoche*0.5  h_b/2];                                  % point en haut � droite
...
NodeSD(1,:) = [delta_encoche*0.5 -h_S/2];                                  % point en bas � gauche
NodeSD(2,:) = [delta_encoche*0.5  h_S/2];                                  % point en haut � gauche
NodeSD(3,:) = [delta_encoche*0.5+delta_dents -h_S/2];                      % point en bas � droite
NodeSD(4,:) = [delta_encoche*0.5+delta_dents  h_S/2];                      % point en haut � droite
...
%% D�placement pour mettre en phase le courant et la tension � vide
...
% Calcul Fem � vide
dx = pastranslation ;
Fem = ((diff(Fluxboba0)/dx)*lm)*(N*2*pi/60); % Voir si il a le m�me r�sultat que les autres.
FemInitA=diff(Fluxboba0)/dt;
FemInitB=diff(Fluxbobb0)/dt;
FemInitC=diff(Fluxbobc0)/dt;
%--------------------------------------------------------------------------

figure('name','Tension � vide et Courant');
yyaxis left
...
Rphase = rho_cu*(nb_spires^2*lcuivre)/(Sfil_cu);                           % R�sistance par bobine

% Pertes Joules
P_Joules = 3*P*Rphase*Ieff^2 ;                                             % Machine triphas� (x3) et P Bobines chaque phase pour P paires de p�les (xP)
...
%--------------------------------------------------------------------------
%% Calculation of FLux at load conditions - M�thode Simplifi�
%--------------------------------------------------------------------------
...
% M�thode Simplifi�
temps = [0 pi/12 pi/6 pi/4]/(P);                                           % Angles par rapport � la partie �lectrique
deplacement = temps.*lm;                                                   % D�placement m�canique par rapport aux angles �lectriques
